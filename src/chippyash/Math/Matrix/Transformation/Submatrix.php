...
class Submatrix extends AbstractTransformation
{
    ...
    public function transform(Matrix $mA, $extra = null)
    {
        $c = count($extra);
        if ($c < 2 || $c > 4) {
            throw new ComputationException('Invalid number of parameters for submatrix operation');
        }
    ...
    function getMatrix()
    {
        ...
        switch ($match) {
            ...
            case 'array,integer,integer':
                list($RL, $j0, $jF) = $args;
                if (($m = count($RL)) == 0) {
                    throw new ComputationException('row indices argument size == 0');
                }
                if (($jF >= $j0) && ($this->mA->columns() >= $jF) && ($j0 >= 0)) {
                    $n = $jF - $j0;
                } else {
                    throw new ComputationException('start and/or end parameters out of bounds');
                }
                ...
                if ($mA->isRational()) {
                    ...
                    return new Matrix($R, false, false, null, false);
                }
                break;
            default:
                throw new UndefinedComputationException('No submatrix for supplied parameters');
                break;
        }
    }
