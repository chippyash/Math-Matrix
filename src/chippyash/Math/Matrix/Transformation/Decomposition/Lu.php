...
class Lu extends AbstractDecomposition
{
     ...
     */
    protected function decompose(NumericMatrix $mA, $extra = null)
    {
    ...
    protected function LUDecomposition(NumericMatrix $mA)
    {
        ...
        for ($j = 0; $j < $n; $j++) {
            ...
            for ($i = 0; $i < $m; $i++) {
                ...
                $kmax = min($i, $j);
                $s = $this->createCorrectScalarType($mA, 0);
                for ($k = 0; $k < $kmax; $k++) {
            ...
            $p = $j;
            for ($i = $j + 1; $i < $m; $i++) {
                if ($LUcolj[$i]->abs()->get() > $LUcolj[$p]->abs()->get()) {
                    $p = $i;
                }
            }
            if ($p != $j) {
...
//        $this->setPermutationMatrix($mA);
    ...
    protected function setLowerProduct(NumericMatrix $mA)
    {
        ...
        $this->set(
                ...
                function() use ($m, $n, $LU, $rcFactor) {
                    ...
                    $mLL = $this->createCorrectMatrixType($ma, $L);
    ...
    protected function setUpperProduct(NumericMatrix $mA)
    {
        ...
        $this->set(
                ...
                function() use ($n, $LU, $rcFactor) {
                    ...
                    $mUU = $this->createCorrectMatrixType($mA, $U);
    ...
    protected function setDeterminant(NumericMatrix $mA)
    {
        if ($this->LU->columns() != $this->LU->rows()) {
            ...
            $this->set('Det', $this->createCorrectScalarType($mA, 0));
            return;
        }

        $det = $this->createCorrectScalarType($mA, $this->pivsign);
        $LU = $this->LU->toArray();
        var_dump($this->LU);exit;
        $calc = new Calculator();
